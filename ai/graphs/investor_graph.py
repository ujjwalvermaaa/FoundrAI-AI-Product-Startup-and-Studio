"""
ai.graphs.investor_graph — LangGraph StateGraph pipeline for investor documentation.
...
Usage:
    from ai.graphs.investor_graph import build_investor_graph, run_investor_graph
    ...
    result = run_investor_graph(state)
"""
...
def _route_after_validation(state: WorkflowState) -> str:
    """Route after validation based on parse outcome and retry count."""
...
def build_investor_graph() -> StateGraph:
    """
    Build the LangGraph StateGraph for investor documentation.
    ...
    graph.add_conditional_edges(
        ...
        {"reflection": "reflection", "repair": "repair", END: END},
    )
...
def run_investor_graph(state: WorkflowState) -> WorkflowState:
    """
    Compile and run the investor documentation graph synchronously.
