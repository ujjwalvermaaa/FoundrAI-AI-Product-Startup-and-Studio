    ...
    <AppPage className="max-w-[1000px]">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="font-display text-3xl font-semibold tracking-tight">
          Global <span className="gradient-text">search</span>
        </h1>
      ...
      <div className="relative">
        ...
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search everything…" className="pl-9 h-11 rounded-xl" autoFocus />
      </div>
      ...
      <div className="space-y-6">
            ...
            {projects.map((p, i) => (
              ...
              >
                <Link to="/projects/$id" params={{ id: p.id }}>
                  <Card className="hover:border-primary/40 transition-all rounded-xl">
                    <CardContent className="p-3 flex items-center gap-3">
                      <span className="size-8 rounded-xl" style={{ background: p.cover }} />
        ...
        <section>
          ...
          <div className="grid gap-2">
            {artifacts.map((a) => (
              <Card key={a.id} className="hover:border-primary/40 transition-all rounded-xl">
                <CardContent className="p-3 flex items-center gap-3">
                  <div className="size-8 rounded-xl bg-accent grid place-items-center"><Sparkles className="size-3.5 text-primary" /></div>
                  <div className="min-w-0">
