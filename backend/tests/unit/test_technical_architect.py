"""
Unit tests for Task 34: Technical Architect Agent and Architecture Graph.

Tests:
  - test_architecture_graph_compiles
  - test_architecture_graph_nodes
  - test_technical_architect_agent_constants
  - test_technical_architect_prompt_files_exist
  - test_architecture_graph_runs_with_mock_llm
  - test_architecture_graph_triggers_repair_on_bad_json
  - test_architecture_graph_fails_gracefully
"""
...
def _make_state(**kwargs):
    return make_initial_state(
        ...
        required_artifacts={
            "product_roadmap": {
                "phases": [
                    {"name": "MVP", "features": ["auth", "validation", "reports"], "timeline": "Q1 2025"},
                ],
            },
        },
        **kwargs,
...
def test_architecture_graph_compiles():
    """build_architecture_graph() must return a StateGraph without raising."""
...
def test_architecture_graph_nodes():
    """Compiled graph must have all 7 expected nodes."""
    from ai.graphs.architecture_graph import build_architecture_graph

    compiled = build_architecture_graph().compile()
...
def test_technical_architect_agent_constants():
    """AGENT_ID, MODULE_KEY, ARTIFACT_TYPE, and SCHEMA must be correct."""
    from ai.agents.technical_architect.agent import AGENT_ID, MODULE_KEY, ARTIFACT_TYPE, SCHEMA
    from ai.schemas.architecture_doc import ArchitectureDoc

    assert AGENT_ID == "technical_architect"
    assert MODULE_KEY == "technical_architecture"
    assert ARTIFACT_TYPE == "architecture_doc"
    assert SCHEMA is ArchitectureDoc


# ── Test 4: prompt files exist ────────────────────────────────────────────────

def test_technical_architect_prompt_files_exist():
    """All 4 prompt template files must exist on disk with non-empty content."""
    prompts_dir = (
        Path(__file__).parent.parent.parent.parent
        / "ai" / "prompts" / "agents" / "technical_architect"
    )
...
@pytest.mark.asyncio
async def test_architecture_graph_runs_with_mock_llm():
...
@pytest.mark.asyncio
async def test_architecture_graph_triggers_repair_on_bad_json():
...
@pytest.mark.asyncio
async def test_architecture_graph_fails_gracefully():
