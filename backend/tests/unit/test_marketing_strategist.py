"""
Unit tests for Task 36: Marketing Strategist Agent and Marketing Graph.

Tests:
  - test_marketing_graph_compiles
  - test_marketing_graph_nodes
  - test_marketing_strategist_agent_constants
  - test_marketing_strategist_prompt_files_exist
  - test_marketing_graph_runs_with_mock_llm
  - test_marketing_graph_triggers_repair_on_bad_json
  - test_marketing_graph_fails_gracefully
"""
...
def test_marketing_graph_compiles():
    """build_marketing_graph() must return a StateGraph without raising."""
...
def test_marketing_graph_nodes():
    """Compiled graph must have all 7 expected nodes."""
    from ai.graphs.marketing_graph import build_marketing_graph

    compiled = build_marketing_graph().compile()
...
def test_marketing_strategist_agent_constants():
    """AGENT_ID, MODULE_KEY, ARTIFACT_TYPE, and SCHEMA must be correct."""
    from ai.agents.marketing_strategist.agent import AGENT_ID, MODULE_KEY, ARTIFACT_TYPE, SCHEMA
    from ai.schemas.marketing_plan import MarketingPlan

    assert AGENT_ID == "marketing_strategist"
    assert MODULE_KEY == "marketing_strategy"
    assert ARTIFACT_TYPE == "marketing_plan"
    assert SCHEMA is MarketingPlan


# ── Test 4: prompt files exist ────────────────────────────────────────────────

def test_marketing_strategist_prompt_files_exist():
    """All 4 prompt template files must exist on disk with non-empty content."""
    prompts_dir = (
        Path(__file__).parent.parent.parent.parent
        / "ai" / "prompts" / "agents" / "marketing_strategist"
    )
...
@pytest.mark.asyncio
async def test_marketing_graph_runs_with_mock_llm():
    ...
    assert len(result["parsed_output"]["channels"]) >= 3
    assert len(result["parsed_output"]["launch_checklist"]) >= 5


# ── Test 6: graph triggers repair on bad JSON ─────────────────────────────────

@pytest.mark.asyncio
async def test_marketing_graph_triggers_repair_on_bad_json():
...
@pytest.mark.asyncio
async def test_marketing_graph_fails_gracefully():
